...
# 임시 데이터
df = pd.read_csv("C:\ITstudy\crawling_musinsa_detail_conc.csv")


# 크롤링 버튼 처리
if st.session_state.button_1:
    with st.spinner('Wait for it...'):
        df = crwaling_fin(d_li)
        st.balloons()
        st.success('Done!')
...
# 전처리 및 새로운 데이터프레임으로 저장
df_v1 = df_v
df_v1.조회수 = df_v.조회수.apply(preproc_view)
df_v1.누적판매 = df_v.누적판매.apply(preproc_sales)

# 데이터 시각화
# 누적판매 상위 10개 브랜드별 판매량 순위+점유율 차트용 데이터프레임 생성
df_brand = pd.DataFrame(df_v1.groupby('브랜드')['누적판매'].sum()).sort_values('누적판매', ascending=False)
df_brand['비율'] = round(df_brand['누적판매']/df_v1['누적판매'].sum()*100, 2)
df_brand = df_brand.reset_index()

# 상위 10개 브랜드용 데이터프레임화-나머지는 기타로 처리
df_brand_10 = df_brand.head(10)
df_brand_10.loc[10] = ['기타', df_brand['누적판매'].tail(len(df_brand)-10).sum(), df_brand['비율'].tail(len(df_brand)-10).sum()]
df_brand_10.index = list(range(1, 12))

# 파이차트 생성
fig1 = px.pie(df_brand_10, values='누적판매', names='브랜드')

# 가격분포표(임시)
fig2 = px.histogram(df_v1, x="가격", range_x=(0, df_v1.가격.quantile(q=0.99, interpolation='nearest')))

# --- 데이터 시각화 스트림릿 구현부 ---

st.subheader("누적판매 상위 10개 브랜드별 점유율 차트 및 순위")
...
with col2: #데이터 프레임
    st.dataframe(data=df_brand_10, use_container_width=False)
